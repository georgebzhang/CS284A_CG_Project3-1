...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) { // Lecture 12, Slide 31, 32
	...
	for (int i = 0; i <= num_samples; ++i) {
		...
		float pdf = 1 / (2 * PI); // Lecture 12, Slide 31
		Ray r = Ray(hit_p + EPS_D * wi_world, wi_world);
		Intersection is;
		if (!bvh->intersect(r, &is)) continue;
		L_out += is.bsdf->get_emission() * isect.bsdf->f(w_out, wi) * wi.z / pdf;
	}
	...
	for (SceneLight* sl : scene->lights) {
		if (sl->is_delta_light()) {
			...
			for (int i = 0; i < ns_area_light; ++i) {
				float pdf; // Would be much better if I wrote a helper function, since this is duplicate code...
				Vector3D wi;
				float distToLight;
				Spectrum s = sl->sample_L(hit_p, &wi, &distToLight, &pdf);
				Vector3D w_in = w2o * wi;
...
Spectrum PathTracer::at_least_one_bounce_radiance(const Ray&r, const Intersection& isect) {
	...
	if (r.depth == max_ray_depth) { // guarantees at least one bounce
	...
	if (coin_flip(1.0 - RRp) && r.depth >= 1) {
		Ray r1 = Ray(hit_p + EPS_D * wi, wi);
		r1.depth = r.depth - 1;
		Intersection isect1;
...
Spectrum PathTracer::est_radiance_global_illumination(const Ray &r) {
	Intersection isect;
	Spectrum L_out;
	Spectrum L_out_direct;
	Spectrum L_out_indirect;
	...
	else if (r.depth == 1) return one_bounce_radiance(r, isect);
	//else return at_least_one_bounce_radiance(r, isect);
	else {
		L_out_direct = zero_bounce_radiance(r, isect) + one_bounce_radiance(r, isect);
		L_out_indirect = at_least_one_bounce_radiance(r, isect) - one_bounce_radiance(r, isect);
	}

	L_out = L_out_direct + L_out_indirect;
	return L_out;
}
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
	...
	int n_converge; // added in Part 5
	if (ns_aa > 1) {
		Spectrum spectrum_total = Spectrum();
		float s1 = 0;
		float s2 = 0;
		for (int n = 0; n <= ns_aa; ++n) { // n refers to nth sample being traced (Part 5 notation)
			Vector2D origin_rand = origin + gridSampler->get_sample();
			Ray r_rand = camera->generate_ray(origin_rand.x / sampleBuffer.w, origin_rand.y / sampleBuffer.h);
			r_rand.depth = max_ray_depth; // added in Part 4
			Spectrum s = est_radiance_global_illumination(r_rand);

			if (n % samplesPerBatch == 0) { // what if we swap the order of checking convergence and incrementing s1 and 2?
				float mu = s1 / (float)n;
				float alpha = sqrt(1.0 / ((float)n - 1.0) * (s2 - (s1 * s1) / (float)n));
				double I = 1.96 * alpha / sqrt((float)n);
		...
		spectrum_result = spectrum_total / n_converge;
	}
	else if (ns_aa == 1) {
		Vector2D origin_mid = origin + Vector2D(0.5, 0.5);
		Ray r_mid = camera->generate_ray(origin_mid.x / (double) sampleBuffer.w, origin_mid.y / (double) sampleBuffer.h);
		r_mid.depth = max_ray_depth; // added in Part 4
		spectrum_result = est_radiance_global_illumination(r_mid);
	}
	...
	sampleCountBuffer[x + y * frameBuffer.w] = n_converge;
...
//	int num_samples = ns_aa;            // total samples to evaluate
//	Vector2D origin = Vector2D(x, y);    // bottom left corner of the pixel
//	int n_converge = 0; // added in Part 5
//	bool converged = false; // added in Part 5
//	if (ns_aa > 1) {
//		Spectrum spectrum_total = Spectrum();
//		float s1 = 0;
//		float s2 = 0;
...
//			s2 += xk * xk;
//
//			if (n % samplesPerBatch == 0 && !converged) { // check for convergence every samplesPerBatch samples
//				float mu = s1 / (float)n;
//				float alpha = sqrt(1.0 / ((float)n - 1.0) * (s2 - (s1 * s1) / (float)n));
//				double I = 1.96 * alpha / sqrt((float)n);
//				if (I <= maxTolerance * mu) {
...
//			spectrum_total += s;
//		}
//		spectrum_result = spectrum_total / ns_aa;
//	}
//	else if (ns_aa == 1) {
//		Vector2D origin_mid = origin + Vector2D(0.5, 0.5);
//		Ray r_mid = camera->generate_ray(origin_mid.x / (double)sampleBuffer.w, origin_mid.y / (double)sampleBuffer.h);
