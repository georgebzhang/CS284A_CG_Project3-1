...
Ray Camera::generate_ray(double x, double y) const {
	...
	double x_cam = (x - 0.5) * 2.0 * tan(radians(hFov) * 0.5);
	double y_cam = (y - 0.5) * 2.0 * tan(radians(vFov) * 0.5);

	Vector3D point_cam(x_cam, y_cam, -1.0);
	Vector3D point_world = c2w * point_cam;
	point_world.normalize();

	Ray r(pos, point_world);
	r.min_t = nClip;
	r.max_t = fClip;
